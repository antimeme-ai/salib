//! Scaffold TCK harness — proves salib's TCK machinery wires
//! end-to-end against a real `#[test]` invocation.
//!
//! No salib-behavior-under-test here; scenarios operate on a
//! trivial `World` (accumulator) so any failure localizes to harness
//! wiring, not salib behavior.
//!
//! Real salib-behavior scenarios (canonical encoding, chain
...
use salib_tck::gherkin::{parse_feature, StepError, SyncRunner};
...
fn scaffold_feature_runs_end_to_end() {
    ...
    // pattern every subsequent salib TCK harness file uses; if
